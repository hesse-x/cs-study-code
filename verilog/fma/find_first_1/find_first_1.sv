...
find_first_1_8bit u_h_8bit(
    .in(in_15_8),
    .all_zero(h_all_zero),
    .out(h_out)
);
find_first_1_8bit u_l_8bit(
    .in(in_7_0),
    .all_zero(l_all_zero),
    .out(l_out)
);
...
assign all_zero = h_all_zero & l_all_zero;
find_first_1_16bit u_h_16bit(
    .in(in_31_16),
    .all_zero(h_all_zero),
    .out(h_out)
);

find_first_1_16bit u_l_16bit(
    .in(in_15_0),
    .all_zero(l_all_zero),
    .out(l_out)
);
...
assign all_zero = h_all_zero & l_all_zero;
find_first_1_32bit u_h_32bit(
    .in(in_63_32),
    .all_zero(h_all_zero),
    .out(h_out)
);

find_first_1_32bit u_l_32bit(
    .in(in_31_0),
    .all_zero(l_all_zero),
    .out(l_out)
);
...
module find_first_1_pad_to_32bit # (
    parameter W = 25
) (
    input  wire [W-1:0]  in,
...
module find_first_1_25bit(
    input  wire [24:0]  in,
    output wire          all_zero,
    output wire [4:0]    out
);

find_first_1_pad_to_32bit#(.W(25)) u_impl (
    .in(in),
    .all_zero(all_zero),
    .out(out)
);
