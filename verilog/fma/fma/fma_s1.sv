...
module fma_mul(
    input a_sign,
    input [common::EXP_WIDTH-1:0] a_exp,
    input [common::MANT_WIDTH-1:0] a_mant,

    input b_sign,
    input [common::EXP_WIDTH-1:0] b_exp,
    input [common::MANT_WIDTH-1:0] b_mant,
...
wire [47:0] result = a_mant * b_mant;

assign t_sign = a_sign ^ b_sign;
assign t_mant = result[47] ? (result >> 1) : result;
wire [common::EXP_WIDTH-1:0] exp_add = a_exp + b_exp;
assign t_exp = (exp_add < 128) ? (8'h00) :
                  (result[47] ? exp_add + 8'sb10000010 : exp_add + 8'sb10000001);
endmodule
